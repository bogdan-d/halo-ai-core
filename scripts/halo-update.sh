...
pre_snapshot() {
    ...
    for svc in llama-cpp lemonade whisper-cpp open-webui vane searxng qdrant n8n kokoro comfyui pipepie goose; do
...
rebuild_services() {
    ...
    if [ -d /srv/ai/lemonade/.git ]; then
        cd /srv/ai/lemonade
        if ! git diff --quiet HEAD@{1} -- package.json src/ 2>/dev/null; then
