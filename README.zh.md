...
| 积木 | 功能 | 状态 |
|------|------|------|
| **nexus vpn** | 零信任WireGuard网状网络（替代SSH网状网络） | [指南 →](docs/wiki/Nexus-VPN.md) |
...
| **shadow** | 完整性 — nexus密钥、文件哈希、mesh健康 |
...
**Lemonade Nexus** — 零信任WireGuard网状VPN。~~SSH混合器已弃用并移除。~~ Nexus是替代方案。

| | SSH网状网络（旧） | Nexus（现在） |
|---|---|---|
| 密钥管理 | 每台机器手动配置 | Ed25519每服务器自动生成 |
| 加密 | 仅SSH | WireGuard ChaCha20-Poly1305隧道 |
| 对等发现 | 无 | UDP gossip协议，自动 |
| 密钥轮换 | 手动 | 使用Shamir自动每周轮换 |
| 治理 | 扁平信任 | 民主制 — Tier 1多数投票 |
| NAT穿越 | 无 | STUN打洞 + 中继回退 |

所有服务绑定到127.0.0.1。Nexus提供加密隧道。*"你不能通过。"*

[Nexus VPN指南 →](docs/wiki/Nexus-VPN.md) · [安全加固 →](docs/SECURITY.md)
