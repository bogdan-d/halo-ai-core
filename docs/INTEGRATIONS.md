...
Four satellite endpoints exist (optional, off by default unless you opt in at
install time). Since 2026-04-19 every halo service runs under **user-systemd**
(unit files in `~/.config/systemd/user/`):

| endpoint | port | systemd unit | what it is |
|---|---|---|---|
| `bitnet_decode --server` | **8080** | `halo-bitnet` | chat completions + model list (OpenAI-compat) — *this is the one* |
| `sd-server` (native HIP) | **8081** | `halo-sd`      | SDXL txt2img/img2img, exposed at `/sd/*` through Caddy |
| `whisper-server`         | **8082** | `halo-whisper` | speech → text (multipart POST) |
| `kokoro-tts` (Bun shim)  | **8083** | `halo-kokoro`  | text → speech (JSON POST → WAV bytes) |
...
## 1b. Image generation (halo-sd on :8081, native HIP SDXL)

```bash
# Start it (if not already running):
systemctl --user start halo-sd

# Simplest local call (loopback):
curl -X POST http://127.0.0.1:8081/sd/txt2img \
  -H 'content-type: application/json' \
  -d '{"prompt":"a 1-bit monster learning to count, cinematic","steps":20}' \
  -o out.png

# Through Caddy + bearer (from another mesh peer):
curl -X POST https://<halo-hostname>.local/sd/txt2img \
  -H "Authorization: Bearer $(cat /etc/caddy/token.secret)" \
  -H 'content-type: application/json' \
  -d '{"prompt":"...","steps":20}' -o out.png
```

Native-HIP port — zero hipBLAS at runtime, zero Python. Typical SDXL 1024×1024
20-step render is ~207 s on Strix Halo; Caddy `/sd/*` read/write timeouts are
set to 15 min to accommodate longer jobs.
...
## 3. Text-to-speech (halo-kokoro on :8083)

```bash
curl -X POST http://127.0.0.1:8083/tts \
  -H 'content-type: application/json' \
...
def speak_listen_reply(wav_path):
    ...
    # 3. synthesize
    audio = requests.post("http://127.0.0.1:8083/tts",
...
## 5b. MCP bridges (stdio, spawned per-session by MCP clients)

Three Model Context Protocol servers live next to halo-ai. Each one is a
stdio JSON-RPC binary that Claude Code / Claude Desktop spawns per session —
nothing listens on a TCP port, nothing phones home:

| bridge | where | transport | what it does |
|---|---|---|---|
| **halo-mcp** | `stampby/halo-mcp` (C++20) | stdio | 17 agent-cpp specialists as `<name>_call` tools. Phase 0 live — stubs return `not implemented`; Phase 1 (`BusBridge`) lands on the agent-cpp message bus and returns real replies. |
| **discord-mcp** | Bun MCP server | stdio | `discord_post_as_specialist` relays under any of 17 specialist identities via an **Echo** webhook (shared webhook URL, per-message `username` + `avatar_url` override). Plus `discord_assign_role` / `discord_remove_role` / `discord_list_roles`, and channel reads. |
| **echo-mcp** | Bun MCP server | stdio | Reddit posting as `echo-halo-ai` via cookie-fetch (no OAuth app). |

Design + phased plan for halo-mcp: [`docs/mcp-nexus-design.md`](mcp-nexus-design.md).

Wire `halo-mcp` into Claude Code:
```json
// ~/.config/claude-code/mcp.json
{
  "mcpServers": {
    "halo-mcp": { "command": "/usr/local/bin/halo-mcp" }
  }
}
```
...
Frosted-glass LAN launcher page at `https://<host>.local/mancave/` — LAN-only
(no bearer gate), static HTML served by Caddy out of `/srv/www/mancave`. Tiles
link to bitnet_decode, halo-sd, whisper, kokoro, Lemonade, Gaia, and anything
you pin. Edit the page directly — no build step, no framework.
